...
class EditQueueTest(BaseTestCase):
    ...
    def test_edit_queue_function(self):
        ...
        response = self.client.put(f"/api/queue/{self.queue.id}", data=json.dumps(edit_attrs),
        ...
        self.assertEqual(response.json(),{'msg': f"Successfully updated the queue '{edit_attrs['name']}' "
    ...
    def test_edit_queue_not_found(self):
        ...
        token = self.login(username='testuser', password='test1234')
        response = self.client.put(f"/api/queue/9999", data=json.dumps(edit_attrs),
                                   content_type='application/json', headers={"Authorization": f"Bearer {token}"})
...
class AddCustomerTestCase(BaseTestCase):
    ...
    def test_post_request_adds_customer(self):
        ...
        response = self.client.post(
            f"/api/queue/new-entry/{self.queue.id}", headers={"Authorization": f"Bearer {token}"})
...
class DeleteQueueTestCase(BaseTestCase):
    ...
    def test_delete_queue_not_found(self):
        ...
        another_queue = Queue.objects.create(name='Another Queue', business=another_business)

        response = self.client.delete(
            f"/api/queue/{another_queue.id}", headers={"Authorization": f"Bearer {token}"})
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.json(), {'msg': "Can't delete another business's queue"})
...
class GetWaitingEntriesTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.entry1 = Entry.objects.create(queue=self.queue, business=self.business, status='waiting',
                                           time_in=timezone.now())
        self.entry2 = Entry.objects.create(queue=self.queue, business=self.business, status='waiting',
                                           time_in=timezone.now())

        # Create a non-waiting entry
        self.entry3 = Entry.objects.create(queue=self.queue, business=self.business, status='completed',
                                           time_in=timezone.now())

    def test_get_waiting_entries(self):
        """Test retrieving waiting entries for a specific queue."""
        token = self.login(username='testuser', password='test1234')  # Adjust to your actual login method
    ...
    def test_get_waiting_entries_no_waiting_entries(self):
        ...
        another_queue = Queue.objects.create(name="Another Test Queue", business=self.business)
