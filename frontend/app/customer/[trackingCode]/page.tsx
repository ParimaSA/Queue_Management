    ...
    <div className="bg-cream2 w-screen h-screen flex justify-center items-center">
      {data.length > 0 ? (
        <div className="bg-white rounded-lg shadow-lg p-10 w-full max-w-full sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl text-center border-2 border-brown"
          style={{ width: '80%', maxWidth: '800px', height: 'auto', resize: 'both', overflow: 'auto' }}
        >
          {data.map((item: EntryData) => (
            <div key={item.id}>
              {/* Business Name */}
              <h3 className="text-yellow-900 text-xl sm:text-2xl md:text-3xl font-bold mb-4 text-brown">{item.business}</h3>
  
              {/* Queue Name and Time In */}
              <div className="text-brown mb-4 text-lg sm:text-lg md:text-xl">
              ...
              {/* Queue Number */}
              <h1 className="font-bold text-amber-900 mb-2 text-4xl sm:text-5xl md:text-6xl">{item.name}</h1>
              ...
              {item.queue_ahead === 0 && (
                <div className="text-green-600 font-bold sm:text-lg md:text-xl mb-2">
              ...
              <button
                ...
                className={`btn btn-error bg-red-600 border-none w-full text-white sm:text-xl md:text-2xl font-semibold mt-3 ${isCancelling ? 'btn-disabled' : ''}`}
              >
